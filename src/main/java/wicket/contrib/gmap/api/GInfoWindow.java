...
public class GInfoWindow extends WebMarkupContainer
{
	 ...
	 *            content to open in info window
     * @param pos
     *            info window position. cannot be null
	 * @return This
	 */
	public GInfoWindow open(GMarker marker, Component content, final InfoWindowPosition pos)
	{
        switch (pos) {
            case SAME: {
                this.marker = marker;
                return open(marker.getLatLng(), new GInfoWindowTab(content));
            }
            case OVER: default: return open(marker, new GInfoWindowTab(content));
        }
	}
